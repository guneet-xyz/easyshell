...
var env = createEnv({
  server: {
    ...
    SESSION_MANAGER_URL: external_exports.string().url(),
    SESSION_MANAGER_TOKEN: external_exports.string().min(1)
  },
...
// ../../packages/session-manager-client/types.ts
var SessionManagerExecResponseSchema = external_exports.object({
  stdout: external_exports.string(),
  stderr: external_exports.string()
});
var SessionManagerIsRunningResponseSchema = external_exports.object({
  is_running: external_exports.boolean()
});
var RunSubmissionMetadataSchema = external_exports.object({
  submission_id: external_exports.number().int().positive(),
  testcase_id: external_exports.number().int().positive(),
  problem_slug: external_exports.string().min(1)
});
var RunSubmissionRequestSchema = external_exports.object({
  image: external_exports.string().min(1),
  input: external_exports.string(),
  metadata: RunSubmissionMetadataSchema
});
var RunSubmissionPostResponseSchema = external_exports.object({
  job_id: external_exports.string(),
  container_name: external_exports.string()
});
var RunSubmissionGetResponseSchema = external_exports.discriminatedUnion("status", [
  external_exports.object({ status: external_exports.literal("running") }),
  external_exports.object({
    ...
    finished_at: external_exports.string()
  }),
  external_exports.object({
...
// ../../packages/session-manager-client/client.ts
var HTTP_STATUS_LOCKED = 423;
var HTTP_STATUS_INTERNAL_SERVER_ERROR = 500;
function createSessionManagerClient(config) {
  ...
  async function exec(args) {
    ...
    const parsed = SessionManagerExecResponseSchema.safeParse(json2);
    if (!parsed.success)
      return {
        status: "error",
        type: "critical_server_error",
  ...
  async function kill(containerName) {
  ...
  async function runSubmission(req) {
  ...
  async function getSubmissionResult(jobId) {
  ...
  async function runSubmissionAndWait(req, opts) {
    const {
      pollIntervalMs = 500,
      maxWaitMs = 6e4,
      postRetries = 3,
...
// utils.ts
var smClient = createSessionManagerClient({
