...
This is a follow-up to a previous problem, <ProblemLink id="grep-version" /> which covered very basic `grep` usage. This problem expands on that and adds a few other regex tricks to the mix.
...
The first two steps are identical to the previous problem. Please refer to <WikiLink id="grep-version" /> for more details.
