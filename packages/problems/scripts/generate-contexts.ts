...
const OUTPUT_DIR = `${env.WORKING_DIR}/build-output`
await rm(OUTPUT_DIR, { recursive: true, force: true })

async function init() {
  await mkdir(`${OUTPUT_DIR}/easyshell-base`, {
    recursive: true,
  })

  await cp(
    `${PROJECT_ROOT}/apps/entrypoint`,
    `${OUTPUT_DIR}/easyshell-base/entrypoint`,
    { recursive: true },
  )

  await writeFile(
    `${OUTPUT_DIR}/easyshell-base/Dockerfile`,
    `
...
async function generateProblemContexts(problem: string): Promise<void> {
  ...
  if (!isStandardProblem(info)) {
    ...
    const IMAGE_DIR = `${OUTPUT_DIR}/${tag}`
...
async function main() {
  ...
  if (args.length === 0) {
    console.error(
      "Provide a problem slug to generate. Provide 'all' to generate all problems.",
    )
    process.exit(1)
  }
  if (args.length > 1) {
  ...
  if (arg === "all") {
    ...
    for (const problem of problems) {
      await generateProblemContexts(problem)
    }
  } else {
    ...
    await init()
    await generateProblemContexts(arg)
  }
