    ...
    children: [
      {
        ...
        children: await (async (): Promise<Array<Test>> => {
          ...
          tests.push({
            name: `(${slug}) assert release workflow exists`,
            callable: async () => {
              const release_workflow = autogeneratedWorkflowPath({
                suffix: slug,
                type: "release",
              })
              const release_workflow_fullpath = `${PROJECT_ROOT}/${release_workflow}`
              const { error } = await neverThrow(
                assertFileExists(release_workflow_fullpath),
              )
              if (error) return error.message
            },
          })
