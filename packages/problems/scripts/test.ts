...
const mustangUrl =
  process.env.MUSTANG_URL ||
  process.env.SESSION_MANAGER_URL ||
  "http://localhost:4000"
const mustangToken =
  process.env.MUSTANG_TOKEN || process.env.SESSION_MANAGER_TOKEN || "token"
...
async function _runSubmissionTest(
  ...
  console.log(
    `[test] submission test: slug=${slug} testcase=${testcase} input=${JSON.stringify(input.slice(0, 80))} expectPass=${pass}`,
  )
...
async function _runLiveEnvironmentTest(
  ...
  console.log(
    `[test] live-env test: slug=${slug} index=${index} input=${JSON.stringify(input.slice(0, 80))} expectPass=${pass} totalPoints=${totalPoints}`,
  )
  const { passed, output } = await runSubmissionAndGetOutput({
    client: mustangClient,
    problemSlug: slug,
  ...
  // Parse "Score: X/Y" from the check.sh output
  const SCORE_REGEX = /Score:\s*(\d+)\/(\d+)/
  const scoreMatch = SCORE_REGEX.exec(output.stdout)
  ...
  console.log(`[test] live-env score: ${score}/${total}`)
